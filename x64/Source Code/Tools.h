...
std::string image_type[] = { "Resources/Image/button_normal.png" , "Resources/Image/button_clicked.png" , "Resources/Image/button_op_sv.png" , "Resources/Image/Cornect.png" , "Resources/Image/opensv.png" , "Resources/Image/openned.png","Resources/Image/discornectted.png", "Resources/Image/cornected_icon.png"};
...
void Draw_Button(sf::RenderWindow& window, sf::Event event, bool& isButtonClicked, int widthpos, int heightpos, int type)
{
    //widthpos = widthpos * resize / 100;
    //heightpos = heightpos * resize / 100;

    // Load textures for button states (normal and clicked)
    sf::Texture Image_Button_Display ;
    if (!Image_Button_Display.loadFromFile(image_type[type]))
    {
    ...
    sf::Sprite Image_Button_Pos(Image_Button_Display);
   ...
   // spriteNormal.setPosition(widthpos, heightpos); // Set position for the button
    ...
    Image_Button_Pos.setPosition(widthpos, heightpos);
    ...
    if (event.type == sf::Event::MouseButtonPressed) {
        ...
        if (event.mouseButton.button == sf::Mouse::Left &&
            Image_Button_Pos.getGlobalBounds().contains(static_cast<float>(event.mouseButton.x), static_cast<float>(event.mouseButton.y))) {
        ...
        sf::FloatRect spriteBounds = Image_Button_Pos.getGlobalBounds();
        ...
        if (spriteBounds.contains(static_cast<float>(mousePos.x), static_cast<float>(mousePos.y))) {
            ...
            std::cerr << "Mouse Released not on Sprite!" << std::endl;
        }
    }

    // Draw the appropriate button state based on click status
    window.draw(Image_Button_Pos);
